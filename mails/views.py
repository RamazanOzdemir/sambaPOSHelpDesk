...
def inbox(request):
    ...
    a = [[e.email, e.subject] for e in all_messages if e.subject == 'deneme']
    print(a,"Query deneme")
    my_messages = []
    all_incoming = []
    for mail in all_messages:
        reply_persons = mail.reply_persons.all()
        if person in reply_persons:
            my_messages.append(mail)

    inbox_mails = serializers.serialize('json', all_messages)
    return render(request, 'mails/inbox.html',
                  {'inbx_mails': inbox_mails, 'all_messages': all_messages, 'my_message': my_messages})
...
#                html_content = "<div dir='ltr'>{}<div dir='rtl'>{}</div></div>".format(form.data['reply'],
#                                                                                       incoming_mail.body_html)
...
#                if 'to' in form.data:
#
#                    html_content = render_to_string('mails/template_message.html',
#                                                    {'to': form.data['to'], 'body': form.data['body'],
#                                                     'name': form.data['name'], 'dept': form.data['dept']})
#                    body_plain = form.data['to'] + form.data['body'] + form.data['name'] + form.data['dept']
#                else:
#                    html_content = '<div><pre>{}</pre></div>'.format(form.data['body_html'])
#                    body_plain = form.data['body_html']
#            connection = get_connection(backend=None, fail_silently=False, username='dene6606@gmail.com',
#                                        password='Kartal1903')
...
#    return render(request, 'mails/create.html', {'form': form, 'formTemplate': formTemplate, 'person': person})
#
#
@login_required(login_url='/account/login/')
def detail(request, uid, reply):
    messageBlock = MessageBlock.objects.get(uid=uid)
    print(messageBlock)
    return render(request, 'mails/detail.html', {'messageBlock': messageBlock, 'reply': reply})
...
#from django.http import JsonResponse, HttpResponse
#from imbox import Imbox
#from .models import Person, OutgoingEmail, IncomingEmail, LastMessage, attachments
#from datetime import datetime
#from django.template.loader import render_to_string
#from . import forms
#from django.core.mail import send_mail, EmailMessage, get_connection
...
#                   'all_persons': all_persons, 'wait_to_reply': wait_to_reply,
#                   'is_super_person': person.is_Super_Person})
#
#
#def new_message(request):
#    imbox = Imbox('imap.gmail.com',
